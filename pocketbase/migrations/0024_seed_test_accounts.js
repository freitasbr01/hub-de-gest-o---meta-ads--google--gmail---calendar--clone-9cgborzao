...
// Semeia dados de trafego (ad_account + campanhas + daily_metrics + ad_sets +
// ads) para contas de TESTE, para que o dashboard e o relatorio por email
// funcionem sem depender da conta demo@hub.com. Cada conta recebe seus proprios
// dados (RLS por user_id). Idempotente: um usuario que ja tenha ad_account e
// pulado. Nao toca nos dados de nenhum outro usuario.
migrate(
  (app) => {
    var TEST_EMAILS = ['fabiano@adapta.org']
    ...
    for (var ei = 0; ei < TEST_EMAILS.length; ei++) {
      var email = TEST_EMAILS[ei]
      var users = app.findRecordsByFilter('_pb_users_auth_', "email = '" + email + "'", '', 1, 0)
      if (users.length === 0) {
        console.log('Test user not found, skipping: ' + email)
        continue
      }
      var userId = users[0].id

      // idempotencia: se ja tem ad_account, nao resemeia
      var existingAcct = []
      try {
      ...
      if (existingAcct.length > 0) {
        console.log('Test account already seeded, skipping: ' + email)
        continue
      }

      var acct = new Record(acctCol, {
        account_id: 'act_test_' + userId,
        name: 'Conta de Teste - Principal',
        currency: 'BRL',
        status: 'active',
        user_id: userId,
      })
      ...
      // metricas diarias dos ultimos 14 dias (ancoradas em now), deterministicas
      ...
      console.log('Seeded test account for ' + email + ': ' + savedCamps.length + ' campaigns')
    }
  },
  (app) => {
    /* seed de teste — sem rollback */
  },
)
