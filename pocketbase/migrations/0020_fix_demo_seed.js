...
// O seed anterior (0018) foi escrito contra um schema antigo: gravava texto em
// campos relation, usava nomes de campos que nao existem mais e nao preenchia
// daily_metrics.campaign_id. Este migration limpa os dados de ads mockados e
// resemeia alinhado ao schema atual, com relacoes corretas.
migrate(
  (app) => {
    ...
    // limpa dados de ads existentes (mock quebrado do 0018 + orfaos)
    var wipe = ['ads', 'ad_sets', 'daily_metrics', 'campaigns', 'ad_accounts']
    for (var w = 0; w < wipe.length; w++) {
      try {
        var old = app.findRecordsByFilter(wipe[w], "id != ''", '', 1000, 0)
    ...
    // metricas diarias dos ultimos 14 dias, distribuicao deterministica
    ...
    // ad sets + ads com os nomes de campos do schema atual
    var adSetNames = [
    ...
    console.log('Demo seed rebuilt: ' + savedCamps.length + ' campaigns with correct relations')
  },
  (app) => {
    /* seed de demo — sem rollback */
  },
)
